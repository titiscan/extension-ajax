component output="no" name="ajaxBase"  {
	...
	public function init( 
			string scriptSrc = variables.instance.SCRIPTSRC
		, 	string cssSrc = variables.instance.CSSSRC
		, 	string loaderSrc = variables.instance.LOADERSRC
		,	string adapter = ''
		,	struct params = {}
		){
		silent {
			...
			savecontent variable="js"{
				...
				if(len(arguments.adapter)){
					writeOutput("<script type='text/javascript' src='#arguments.adapter#'></script>")
				}
			}
		}
	...
	*/
	...
	<!--- StripWhiteSpace --->	
	public any function stripWhiteSpace(string str)output="no" returntype="string" hint="Strips whitespace outside tags from string" {
