component extends="lucee.core.ajax.AjaxBase" output="no" {
	...
	public any function init(required boolean hasEndTag,component parent)returntype="void" hint="invoked after tag is constructed"{
		variables.hasEndTag = arguments.hasEndTag;
		super.init();
	}

	public function onStartTag(struct attributes, struct caller) {
		silent{
			<!--- check --->
	...
	public any function doBind(struct attributes,struct caller)returntype="void"{
		silent{
			var js = "";
